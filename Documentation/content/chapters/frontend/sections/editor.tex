...
The editor or designer is undeniably the most complex and core component of the frontend.
...
The full use cases of the developed designer can be viewed on Figure \ref{fig:designer-use-cases}.
...
The editor with some example components inserted is displayed on Figure \ref{fig:editor-base}.
...
This is a resizeable canvas to which users can drag and drop the editor's components to.
It
On hovering the components an outline appears for the selection shown the name of the component, a mover icon prompting that it can be moved, and a delete button to remove it from the page.
...
The middle section of the top bar has viewport controls that resize the editable area to see how the page would look like in desktop (default), tablet and mobile sized screens.
...
The third section of the top bar houses a copy button for quick copying of the editor content, a load button that allows rebuilding the page from the copied content through a dialog.
...
The steps described above however only take care of the JSON, however the static HTML has to be somehow generated as well.
...
The third tab shows the available component for use using small cards with icons, description, and tooltips.
From here they can be dragged and dropped to the designer area.

I implemented nine components as a part of the thesis.
A generic typography component for creating texts, which can use three different fonts (sans, serif and mono), weight, italics, underlining and strike through among many.
A button component which allows connecting the pages of a given site by using relative routes (e.g., \texttt{/about}) or by providing absolute links to external pages.
A card component similar to the card view of schemas and sites.
Simple badge and separator components. A calendar component displaying a specific date.
An image component for external images, for which size and width can be either fixed or auto or a combination of it (e.g., fixed width and auto height).
A video component for linking external videos, which also automatically converts YouTube links to their embedded versions.
Finally, a grid component for organization which allows a row or column layout and setting the amount of cells. They can also be nested inside each other.
...
To finish off on Figure \ref{fig:editor-variant} the previously mentioned reaction to the disabled or unselected states can be viewed, on Subfigure \ref{subfig:editor-disabled} and Subfigure \ref{subfig:editor-no-selection} respectively.
