...
As with the previous features, I want to present a use-case diagram of them on Figure \ref{fig:schema-use-cases}.
...
Schemas can be used as templates for pages, that I will mention the details of later.
...
On Figure \ref{fig:schema-list} a list view of schemas can be seen.
...
The schema edit and create page is basically the same, and it is a simple form, that can be viewed on Figure \ref{fig:schema-edit}.
...
Once again, a use case diagram of site related features is visible on Figure \ref{fig:site-use-cases}.
...
The detail view of the site appears on Figure \ref{fig:site-detail}.
...
On the other hand Subfigure \ref{subfig:site-pages} show the pages of the site, where their detail view can be visited by clicking on a card, or a new one can be created with the provided button.
...
This is because we stay on the same view, instead of navigation away, which would normally trigger SWR's re-fetch mechanism.
...
Since pages can be based of schemas, the creation form needs to be able to provide the ability to select a schema as the base content for a page.
...
At the end of the form I show to the user a review tab and the process can also be reset here if needed.
The page edit form is similar to the schema edit form on Figure \ref{fig:schema-edit}.
The detail view of the pages also contain a preview of the content for easier identification.
