...
The first obvious question one may have is why is this a separate application instead of part of the backend.
...
As with the other two application I set up a Docker init based Dockerfile and compose file for containerization purposes.
...
This is seen on Figure \ref{fig:proxy-openapi}.
...
Since Fastify is also unopinionated about the code structure, I decided to take inspiration from NestJS's module based structure, which provides a maintainable and extendible base for this proxy application.
This structure can be seen on Figure \ref{fig:proxy-struct}.
...
The \texttt{kube} folder contain an example configuration file for connecting to the cluster, which has to be copied and set accordingly.
Inside \texttt{src} the \texttt{routes} folder contains files which list the routes of the application, in this case the CRUD + restart routes for deployments.
The \texttt{assets} contain the \texttt{index.css} file which provides the look of the websites, while \texttt{types} organize global TypeScript types, as module specific ones live inside their respective folders.
...
Lastly I set up the server up in the \texttt{src/index.tsx} file where I registered the routes to the server instance.

The API module configures the Kubernetes client library and exports the core, apps, and objects API for other modules to use and create resources.
...
All other Kubernetes resources get the same label selector defined so to easily identify all cluster resources belonging to the same site.
...
This contains the name of the site, which in this case is equal to the subdomain as it is a unique identifier for it, array of its pages, and the namespace to deploy it to.
...
Next I use an init container to fetch the HTML files from the provided URL's of the S3 storage and mount the result to the actual container inside the pods.
The container itself is a simple NGINX web server serving the site and its pages statically.
The pod also has limits set as 500m for CPUs and 500Mi for memory.
...
The full sequence diagram for the creation of the website's deployment is on Figure \ref{fig:proxy-sequence}.
