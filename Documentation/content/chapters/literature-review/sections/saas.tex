...
In order to develop a software as a service (SaaS) application suite we first have to explore what a SaaS entails.
...
The NIST (National Institute of Standards and Technology) defines five essential characteristics, four deployment models and three service models for the cloud. \cite{mell2011nist}
...
\begin{itemize}
  ...
  \item Rapid elasticity: capabilities are scalable and reassignable on demand.
  \item Measured service: resource usage is monitored, controlled and automatically optimized.
\end{itemize}

The aforementioned deployment models are:
\begin{itemize}
  \item Private cloud: the cloud is used by a single organizational entity
  \item Community cloud: the infrastructure is used by a community composed of multiple organizations or
  \item Public cloud: the cloud is open for use to the public.
  \item Hybrid cloud: the cloud is some mix of the models mentioned before.
\end{itemize}

All the above deployment models may be either on or off premise and additionally owned or managed by the organization(s) using them or third parties, and c.
...
\begin{itemize}
  ...
  \item Platform as a Service (PaaS): the provided resources allow the customer to deploy applications to the cloud providers platform without configuring the underlying computing infrastructure.
...
SaaS applications hence are also web applications and built with web technologies and frameworks.
...
The methods usually differ by how separated each tenants share is on a database, data model and infrastructure level. \cite{tsai_bai_huang_2014}
...
Customization can range from interface level differences to full on tailoring of the tenants share of the application down to the platform or even infrastructure level.
