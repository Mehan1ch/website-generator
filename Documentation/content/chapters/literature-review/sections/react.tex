...
Since SPA-s only serve one HTML file from the backend they need some way to know which component to render for which web route. For this purpose developers use one of the many available routing libraries.
...
The main difference is in folder based grouping the file serving exact path of the folder (e.g., \texttt{/posts}) must be named \texttt{index.tsx} while layout routes should be named \texttt{route.tsx}.
