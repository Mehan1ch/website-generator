...
React is also unsurprisingly unopinionated about testing, and there exists many robust packages that solve this issue.
...
The latter has the potential problem that the simulated environment is not a one on one match with the live and the DOM and CSS get handled a bit differently.
...
The first two handles starting and stopping the mocked worker server (more on this later), while the third resets the handlers of the server and clears a test query client.
...
The wrapper ensures that all mocked paths are valid according to the OpenAPI schema, and mocked requests' and responses' structure are correct.
...
All that remained at this point is implementing the test cases.
...
The first tests the authentication flow by registering a new user, logging out and back in, changing user information and password and finally logging back in with the new credentials after the automatic logout.
The other end-to-end test checks the site management flow.
It includes logging in, navigating to the sites view, creating a new site, viewing it in detail view, opening its home page in the designer, re-saving the design, navigating back to the site detail view and deploying it.

On Figure \ref{fig:frontend-test} the testing browser environment with the successfully run tests can be seen.
